...
export default function Review({ review }: { review: AnimeReviews }) {
    ...
    return (
        <div className="text-white px-4 py-2 mt-4">
            <Link to={review.user.url} className="flex flex-row items-center w-fit hover:bg-gray-700 rounded-md px-2 py-2 transition-all">
            ...
            {review.review.length > 350 && !showMore ? (
                <p className="dark:text-white text-lg cursor-pointer" onClick={() => setShowMore(true)}>Show more</p>
            ):
                (
                    <p className="dark:text-white text-lg cursor-pointer" onClick={() => setShowMore(false)}>Show less</p>
                )
            }
            <div className="flex flex-row justify-between items-center mt-1.5">
                <p className={`sm:text-lg md:text-2xl font-bold ${scoreClasses.find((scoreClass) => scoreClass.score === review.score)?.className ?? 'text-gray-500'}`}>Score: {review.score}</p>
                <p className="text-base dark:text-gray-300 ">Date: {review.date.split('T')[0]}</p>
            </div>
        </div>
